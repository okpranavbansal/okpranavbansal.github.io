    ...
    if (currentSection === "Professional Experience") {
      if (line.startsWith("### ")) {
        ...
        if (line.startsWith("_") && line.endsWith("_")) {
          ...
          let subtitlePart = pipeParts[1]
            ? pipeParts[1].replace(/_/g, "").trim()
            : "";
          ...
          if (
            promoMatch &&
            (promoMatch[2].includes("promoted") ||
              promoMatch[2].includes("Converted"))
          ) {
            titlePart = promoMatch[1].trim();
            subtitlePart = subtitlePart
              ? `${promoMatch[2]} · ${subtitlePart}`
              : promoMatch[2];
          }
...
try {
  ...
  if (!fs.existsSync(MARKDOWN_PATH)) {
    if (optionalSync && fs.existsSync(OUTPUT_PATH)) {
      console.warn(
        `Warning: Markdown resume not found at ${MARKDOWN_PATH}; using committed ${OUTPUT_PATH}`,
      );
      process.exit(0);
    }
